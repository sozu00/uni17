...
class Abin {
	public:
		...
		T& elemento(nodo n);// acceso a elto, lectura/escrituranodo raizB() const;
...
template <typename T>
const typename Abin<T>::nodo Abin<T>::NODO_NULO(-1);template <typename T> inline Abin<T>::Abin(size_t maxNodos) : nodos(new celda[maxNodos]), maxNodos(maxNodos), numNodos(0){}
...
void Abin<T>::eliminarHijoDrchoB(Abin<T>::nodo n){
	...
	if (hdrcho != numNodos-1){
		...
		if (nodos[nodos[hdrcho].padre].hizq == numNodos-1)
			...
			nodos[nodos[hdrcho].padre].hder = hdrcho;// Si el nodo movido tiene hijos,
			// actualizar la posición del padre
...
template <typename T>
nline typename Abin<T>::nodo Abin<T>::raizB() const{
...
template <typename T>
bin<T>& Abin<T>::operator =(const Abin<T>& a){
...
template <typename T>
int prof(Abin<T>::nodo n){
	int prof=0, pr=n;//PosicionRelativa
	int medio = (maxNodos+1)/2;

	while(pr != medio){
		++prof;
		pr = (pr > (medio+1)/2) ? medio-pr : pr;
		medio = (medio+1)/2;
	}
	return prof;
}
